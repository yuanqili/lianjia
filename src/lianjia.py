...
class Lianjia:
    ...
    def __init__(self, location):
        ...
        self.total_pages = self.scrap_total_pages()
        self.__current_page = 1
        self.__houses = []

    def scrap_total_pages(self):
        url = f'{Lianjia.base_url}/{self.location}'
    ...
    def scrap_page(self, page_no):
        url = f'{Lianjia.base_url}/{self.location}/pg{page_no}'
        response = requests.get(url)
        soup = BeautifulSoup(response.content.decode(), 'html.parser')
        ...
        for item in items:
            try:
                parsed_item = self.parse_page(item)
    ...
    def __next__(self):
        if self.__current_page > self.total_pages:
            raise StopIteration()
        if self.__houses:
            return self.__houses.pop(0)
        self.__houses = self.scrap_page(self.__current_page)
        self.__current_page += 1
        return self.__houses.pop(0)
...
if __name__ == '__main__':
    location = 'aoti3'
    db = TinyDB(f'{location}.json')
    lianjia = Lianjia(location)
    for house in lianjia:
        print(house)
